...
use crate::models::{NewTenant, Tenant, UpdateTenant};
...
pub fn get_all_tenants() -> Result<Vec<Tenant>, Error> {
    ...
    tenants.filter(deleted_at.is_null()).load::<Tenant>(conn)
}
...
pub fn create_tenant(new_tenant: NewTenant) -> Result<Tenant, Error> {
    ...
    let tenant_data = (id.eq(tenant_id), display_name.eq(new_tenant.display_name));

    diesel::insert_into(tenants)
        .values(tenant_data)
        .execute(conn)?;

    tenants.order(created_at.desc()).first(conn)
}
...
pub fn update_tenant(tenant_id: &str, tenant_update: UpdateTenant) -> Result<Tenant, Error> {
    ...
    tenants.filter(id.eq(tenant_id)).first(conn)
}
...
pub fn delete_tenant(tenant_id: &str) -> Result<usize, Error> {
    ...
    diesel::delete(tenants.filter(id.eq(tenant_id))).execute(conn)
}
